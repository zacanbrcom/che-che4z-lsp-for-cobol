...
public class CobolLanguageEngineFacade implements LanguageEngineFacade {
  ...
  private Map<String, List<Location>> convertPositions(Map<String, Collection<Position>> source) {
    return source.entrySet().stream().collect(toMap(Map.Entry::getKey, convertPositions()));
  }

  private Function<Map.Entry<String, Collection<Position>>, List<Location>> convertPositions() {
    return it ->
        it.getValue().stream()
